...
// Middleware to verify admin access (simple - you can enhance with actual admin login)
function authenticateAdmin(req, res, next) {
    ...
    try {
        const decoded = jwt.verify(token, JWT_SECRET);
        // Check if user has admin role (you can add an 'is_admin' column to vendors table)
...
app.get('/api/admin/payouts/vendors/pending', async (req, res) => {
    try {
...
app.get('/api/admin/payouts/riders/pending', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT rp.*, 
    ...
    try {
        ...
        if (type === 'vendors') {
            result = await pool.query(`
                SELECT vp.*, v.name as vendor_name, v.email as vendor_email, 'vendor' as user_type
                FROM vendor_payouts vp
                JOIN vendors v ON vp.vendor_id = v.id
            ...
            result = await pool.query(`
                SELECT rp.*, r.name as rider_name, r.email as rider_email, 'rider' as user_type
                FROM rider_payouts rp
                JOIN riders r ON rp.rider_id = r.id
...
// Process a vendor payout (mark as completed)
app.patch('/api/admin/payouts/vendors/:payoutId/process', async (req, res) => {
    ...
    try {
        ...
        const payoutResult = await client.query(
            `SELECT vp.*, v.mobile_money_number, v.email 
             FROM vendor_payouts vp
             JOIN vendors v ON vp.vendor_id = v.id
             WHERE vp.id = $1 AND vp.status = 'pending'`,
            [payoutId]
        );
        ...
        // Record transaction in vendor_transactions
        await client.query(
...
// Process a rider payout (mark as completed)
app.patch('/api/admin/payouts/riders/:payoutId/process', async (req, res) => {
    const { payoutId } = req.params;
    const { transaction_reference, notes } = req.body;
    ...
    try {
        ...
        const payoutResult = await client.query(
            `SELECT rp.*, r.mobile_money_number, r.email 
             FROM rider_payouts rp
             JOIN riders r ON rp.rider_id = r.id
             WHERE rp.id = $1 AND rp.status = 'pending'`,
            [payoutId]
        );
...
app.patch('/api/admin/payouts/vendors/:payoutId/reject', async (req, res) => {
    const { payoutId } = req.params;
    ...
    try {
        ...
        await client.query(
            ...
            [payout.vendor_id, payout.amount, 
             `Payout of ₵${payout.amount} was rejected. Reason: ${reason || 'Not specified'}`]
        );
...
app.get('/api/admin/payouts/stats', async (req, res) => {
    try {
        // Get total paid out this month
